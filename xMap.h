...
V xMap<K, V>::put(K key, V value) {
  ...
  for (auto list_I : list_of_index){
      if (keyEQ(list_I -> key, key)){
          V oldValue = list_I -> value;
          list_I -> value = retValue;
          return oldValue;
      }
  }
  Entry* newKey =  new Entry(key, value);
  list_of_index.add(newKey);
  ...
  // TODO YOUR CODE IS HERE
  // std::cout << "Contents of list_clashes: ";
  // for (auto clash : this->list_clashes) {
  //   std::cout << clash << " ";
  // }
  // std::cout << std::endl;
  return retValue;
...
bool xMap<K, V>::containsValue(V value) {
  // TODO YOUR CODE IS HERE
  for (int idx = 0; idx < capacity; idx ++){ //co the la count??
...
DLinkedList<int> xMap<K, V>::clashes() {
  ...
  for (int idx = 0; idx < capacity; idx++){
      ...
      list_clashes.add(count_clashes);
  }
  return this->list_clashes; 
}
