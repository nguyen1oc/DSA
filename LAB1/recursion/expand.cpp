string expand(string s) {
    if(s.length()==0) return "";
    else if((s[0]>='a' && s[0]<='z') || (s[0]>='A' && s[0]<='Z')) return s[0]+expand(s.substr(1));
    else if(s[0]>='2'&&s[0]<='9'){
        if(s[1]=='('){
            s[0]--;
    ...
    }else if(s[0]=='0') return expand(s.substr(2+expand(s.substr(2)).length()));
    else if(s[0]==')') return "";
    else if(s[0]=='('){
        ...
        else return 'a'+expand(','+s.substr(2));
    }
}
